...
const DriverCreatePage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        setLoading(true);
        const [g, b, m, e, s, f, st, v, bc] = await Promise.all([
        ...
        showAlert("Failed to load setup data.", "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    try {
      ...
      console.log("[DRIVERS] Submitting form data:", ...Array.from(formData.keys()));
      ...
      if (response.data.success) {
        showAlert("Driver onboarding successful!", "success");
        navigate("/drivers");
      }
    } catch (error: any) {
      showAlert(error.response?.data?.message || "Failed to onbord driver", "error");
    }
  };
      ...
      <div className="px-6 pt-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <div className="flex items-center gap-2 text-[11px] font-800 text-muted uppercase tracking-widest px-1">
             ...
             <span>Personnel</span>
             <span className="text-slate-300">/</span>
             <span className="text-primary-dark">Onboard Driver</span>
           </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-[1100px] mx-auto space-y-6 pb-24">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <SectionCard icon={IdCard} title="Personal Identity Profile">
              <div className="space-y-5">
                ...
                <FileUpload label="Profile Image" name="profile_photo" register={register} errors={errors} />
              </div>
            </SectionCard>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             ...
             <SectionCard icon={HardDrive} title="Compliance Repository">
                <div className="grid grid-cols-2 gap-4">
                   <FileUpload label="Driving Auth" name="driving_license" register={register} errors={errors} />
                   <FileUpload label="Identity Card" name="aadhaar_card" register={register} errors={errors} />
                </div>
             </SectionCard>
          </div>
