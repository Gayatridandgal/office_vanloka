...
functions_1.app.http("staffById", {
    ...
    handler: async (req, ctx) => {
        ...
        try {
            ...
            if (req.method === "DELETE") {
                const { rowCount } = await client.query(`DELETE FROM vl_staff_members WHERE id = $1 AND org_id = $2`, [id, auth.user.org_id]);
                if (rowCount === 0)
            ...
            const { rows } = await client.query(`UPDATE vl_staff_members SET
