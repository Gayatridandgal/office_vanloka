    ...
    handler: async (req, ctx) => {
        ...
        try {
            ...
            if (req.method === "GET") {
                ...
                const { rows } = await client.query(`SELECT id, first_name, last_name, gender, email, mobile_number,
           ...
           LIMIT $5 OFFSET $6`, [auth.user.org_id, status ?? null, city ?? null, search ?? null, pageSize, (page - 1) * pageSize]);
           ...
           WHERE org_id = $1
             ...
             AND ($4::text IS NULL OR
                  ...
                  mobile_number ILIKE '%' || $4 || '%' OR
                  employee_id ILIKE '%' || $4 || '%' OR
                  email ILIKE '%' || $4 || '%')`, [auth.user.org_id, status ?? null, city ?? null, search ?? null]);
            ...
            const { rows } = await client.query(`INSERT INTO vl_drivers (
