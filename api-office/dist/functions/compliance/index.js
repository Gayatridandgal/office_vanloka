...
functions_1.app.http("compliance", {
    ...
    handler: async (req, ctx) => {
        ...
        try {
           ...
           LIMIT $5 OFFSET $6`, [auth.user.org_id, entityType ?? null, status ?? null, search ?? null, pageSize, (page - 1) * pageSize]);
                const { rows: count } = await client.query(`SELECT COUNT(*)
           FROM vl_compliance c
           WHERE c.org_id = $1
             AND ($2::text IS NULL OR c.entity_type = $2)
             AND ($3::text IS NULL OR c.status = $3)
             AND ($4::text IS NULL OR c.document_type ILIKE '%' || $4 || '%')`, [auth.user.org_id, entityType ?? null, status ?? null, search ?? null]);
            ...
            const { rows } = await client.query(`INSERT INTO vl_compliance (
