      ...
      if (req.method === "GET") {
        ...
        const { rows } = await client.query(
          `SELECT id, vehicle_number, model, manufacturer, vehicle_type, year, fuel_type,
           ...
           LIMIT $5 OFFSET $6`,
          [auth.user.org_id, status ?? null, vehicleType ?? null, search ?? null, pageSize, (page - 1) * pageSize],
        );
          ...
          `SELECT COUNT(*)
           ...
           WHERE org_id = $1
             ...
             AND ($4::text IS NULL OR
                  ...
                  model ILIKE '%' || $4 || '%' OR
                  assigned_driver ILIKE '%' || $4 || '%')`,
          [auth.user.org_id, status ?? null, vehicleType ?? null, search ?? null],
        );
