      ...
      if (req.method === "GET") {
        ...
        const { rows } = await client.query(
          `SELECT id, employee_id, first_name, last_name, gender, email, phone,
           ...
           LIMIT $5 OFFSET $6`,
          [auth.user.org_id, status ?? null, department ?? null, search ?? null, pageSize, (page - 1) * pageSize],
        );
          ...
          `SELECT COUNT(*)
           ...
           WHERE org_id = $1
             ...
             AND ($4::text IS NULL OR
                  ...
                  employee_id ILIKE '%' || $4 || '%' OR
                  phone ILIKE '%' || $4 || '%')`,
          [auth.user.org_id, status ?? null, department ?? null, search ?? null],
        );
